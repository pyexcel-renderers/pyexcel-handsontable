...
class HandsonTable(Renderer):
    ...
    def render_book(
        self, book, embed=False, css_url=CSS_URL, js_url=JS_URL, **keywords
    ):
        """
    ...
    def _parse_book(self, book, styles=None, **keywords):
        ...
        for sheet in book:
            ...
            if sheet.name in styles:
                ...
                handson_sheet["style"] = _dump_dict(sheet_style)
...
class HandsonTableInJupyter(HandsonTable):
    def get_io(self):
        ...
        setattr(io, "_repr_html_", partial(lambda s: s.getvalue(), io))
        return io
    ...
    def render_book_to_stream(
        ...
        **keywords
        ...
        book_data = self._parse_book(book, **keywords)
        __css_file__ = os.path.join(
            _get_resource_dir("templates"),
